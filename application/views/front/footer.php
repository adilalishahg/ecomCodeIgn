                  ...
                  <div class="row">
                     ...
                     <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                        <div class="tp-footer-widget footer-col-2 mb-50">
                           ...
                           <div class="tp-footer-widget-content">
                              <ul>
                                 <li><a href="<?= base_url()?>order/order_track">Track Orders</a></li>
...
$("#update_cart-form").submit(function(e) {
	var formMessages = $('.alert'); 
	var form = $('#update_cart-form');
$(formMessages).text('');
...
$('.tp-cart-action-btn').on('click', function(e) {
	...
	var hrefValue = $('.tp-cart-action-btn').data('href');
...
// Update total price when quantity changes
$('.tp-product-quick-view-btn').on('click', function(e) {
	e.preventDefault()
	 
	var dataId2 = $(this).data('view_id');
	var slug = $(this).data('slug');
		let formMessages = $('.alert'); 
	// let dataId = $(this).data('view_id');
	console.log(dataId2) 
	$.ajax({
		type: 'POST',
		url: 'product/'+dataId2,
		data: {id:dataId2,slug:slug,json:true}
	})
	.done(function(response) {
		let res = (JSON.parse(response));
		// Make sure that the formMessages div has the 'success' class.
	...
	$('.modal-content').html('') 
	let js = '';
	 js+=` <div class="tp-product-modal-content d-lg-flex align-items-start">
                        ...
                        <nav>
                           <div class="nav nav-tabs flex-sm-column " id="productDetailsNavThumb" role="tablist">
                              <button class="nav-link active" id="nav-1-tab" data-bs-toggle="tab" data-bs-target="#nav-1" type="button" role="tab" aria-controls="nav-1" aria-selected="true">
                                 <img src="assets_front/images/products/${res.pro_main_image}" alt="">
                              </button>
                     ...
                     <div class="tp-product-details-wrapper">
                        ...
                        <h3 class="tp-product-details-title">${res.pro_name}</h3>
                        ...
                        <p>${res.description}... <span>See more</span></p>
   
                        <!-- price -->
                        <div class="tp-product-details-price-wrapper mb-20">
                           <span class="tp-product-details-price old-price">${res.mrp}</span>
                           <span class="tp-product-details-price new-price">${res.selling_price}</span>
                        </div>
...
$('.tp-product-add-cart-btn').on('click', function(e) {
	e.preventDefault();
	var formMessages;  
	
	// Assuming your anchor tag has an id attribute 'myAnchor'
	var dataId = $(this).data('id');
    var hrefValue = $(this).data('route');

	 var element = $('[data-alert_id="'+dataId+'"]');
	 if ($("#list-tab-pane").hasClass("active") && $("#list-tab-pane").hasClass("show")) {
 		formMessages = element
	  }else{
		formMessages= $('.alert');
	  }
	 $.ajax({
